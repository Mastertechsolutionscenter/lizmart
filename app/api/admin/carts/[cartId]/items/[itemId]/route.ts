...
export async function PATCH(req: Request, { params }: { params: Promise<{ cartId: string }> }) {
  try {
    const { cartId } = await params;
    ...
    console.error("PATCH /api/carts/[id]", err);
    if ((err as any)?.code === "P2025") return NextResponse.json({ error: "Cart not found" }, { status: 404 });
    return NextResponse.json({ error: "Failed to update cart" }, { status: 500 });
  }
}

export async function DELETE(req: Request, { params }: { params: Promise<{ cartId: string }> }) {
  try {
    const { cartId } = await params;
    ...
    console.error("DELETE /api/carts/[id]", err);
    if ((err as any)?.code === "P2025") return NextResponse.json({ error: "Cart not found" }, { status: 404 });
